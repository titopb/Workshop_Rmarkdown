...
date: "First version: 28 February 20233. This version: `r format(Sys.Date(), '%d %B %Y')`."
...
# create variable age based on since
# drop since from the dataframe
df <- df_raw %>%
      mutate(subfield = as.factor(subfield), 
             english = as.factor(english),
             age = 2023 - since) %>%
      select(-since) %>% 
  filter (origin != "Oceania") #retirar jornais da Oceania
...
[jane.doe\@random.edu](mailto:jane.doe@random.edu){.email}. Website:
www.janedoe.com.
...
Vestibulum, **sociis ipsum**. Massa, sed ac eu fringilla sed
luctus sodales, sit porttitor non. Iaculis litora, accumsan facilisis
lacinia laoreet rhoncus pellentesque sed. At, viverra enim proin quis
neque efficitur. Quam sit est sed at ut aliquam. Condimentum efficitur
...
mollis magnis mauris platea velit natoque *consequat in cubilia*.

<!-- paragraph 3 -->

Eu sodales phasellus nullam ornare, est ligula.[^1]
...
integer.^[Non at cubilia ut at nisi].

<!-- footnote 1 -->

[^1]: In nunc eu libero arcu posuere varius. Donec nascetur sagittis volutpat
...
<!-- paragraph 4 -->
[Literature Review](#literature-review), dolor eget orci integer gravida. Efficitur et augue facilisis turpis, neque pharetra venenatis ut. Tempus ut sapien dui
...
Varius R^2^ natoque dolor nisl donec nostra id dictum
...
Nisl sed ridiculusn 12--13, ut habitant vitae tempus
...
torquent, vitae, ut tempus. Sed dictum non hendrerit in
...
Sit nunc auctor at id diam [@bennett2015]. Tempus platea eu.
...
congue augue id semper magna libero [@gilbert2019 216].
...
sagittis velit [for example, see @bennett2015].
...
turpis in [@bennett2015; @delgado2010].
...
There are `r nrow(df)` observations in the dataset. Egestas
...
pretium mus malesuada pellentesque \autoref{b_plot}

<!-- figure 2 -->

```{r, barplot, fig.align='center', out.width="75%", fig.cap="Geographic distrubition of journals. \\label{b_plot}"}

#frequencia journais

ggplot(data = df, aes(x = forcats::fct_infreq(origin))) +
  geom_bar() +
  theme_bw() +
...
Ultrices <!-- inline equation --> a2 = b2 + c2 laoreet sollicitudin
...
Ultrices rhoncus non risus mauris est nunc \autoref{r_equation}:
...
\autoref{s_table} volutpat sapien curae non tortor at velit, amet. Eget velit
...
\autoref{s_plot}lectus mauris, risus nisl fames vitae aptent ac sed. At et
...
ggplot(data = df, aes(x = h5_median, y = h5_index, color = subfield)) +
       geom_point() +
       facet_wrap(. ~ branch) +
...
\autoref{r_table} Mauris erat nec scelerisque aptent praesent in, luctus ac
...
journals publish in a given year is associated with a `r round(coef(summary(lm1))["issues", "Estimate"], digits = 1)` increase
(standard error = `r round(coef(summary(lm1))["issues", "Std. Error"], digits = 1)`) in their H5 Index.
